...
const calls = [
  ...
  { dataHora: "18/01 11:30", numero: "(11) 98000-2222", duracao: "4:00", status: "Atendida", qualificacao: "Warm", campanha: "Google - Brand Search", contato: "José Carlos" },
...
const columns: ColumnDef<Call, any>[] = [
  ...
  { accessorKey: "numero", header: "Número", cell: ({ getValue }) => <span className="font-mono text-xs">{getValue() as string}</span> },
  { accessorKey: "duracao", header: "Duração", cell: ({ getValue }) => <span className="font-mono tabular-nums">{getValue() as string}</span> },
  ...
  { accessorKey: "qualificacao", header: "Qualificação", cell: ({ getValue }) => {
...
const CallTrackingPage = () => (
  <div className="space-y-6 max-w-[1600px]">
    <PageHeader title="Call Tracking" subtitle="Monitoramento de ligações e atribuição a campanhas" />
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
      ...
      <KPICard title="Duração Média" value="4:52" change={3.1} sparkData={[3,4,3,5,4,5,5]} />
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-xl surface-glow p-5">
      <p className="text-xs font-medium text-muted-foreground uppercase tracking-widest mb-4">Ligações nos últimos 30 dias</p>
    ...
    <DataTable data={calls} columns={columns} searchPlaceholder="Buscar por número ou contato..." />
  </div>
);
