...
const OfflineConversionsPage = () => (
  <div className="space-y-6 max-w-[1600px]">
    <PageHeader title="Conversões Offline" subtitle="Upload de vendas offline e match com campanhas" />

    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="border-2 border-dashed border-border rounded-xl p-12 flex flex-col items-center justify-center text-center hover:border-primary/50 transition-colors cursor-pointer">
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-card rounded-xl surface-glow p-5">
      <p className="text-xs font-medium text-muted-foreground uppercase tracking-widest mb-4">Último Upload — Match Report</p>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-success/10 border border-success/20 rounded-xl p-4">
      <p className="text-sm flex items-center gap-2"><ArrowUpCircle className="h-4 w-4 text-success" /> Com conversões offline, seu ROAS real é <span className="font-semibold text-success">3.8x</span> (antes mostrava <span className="text-muted-foreground">2.1x</span>)</p>
    </motion.div>

    <div>
      <p className="text-xs font-medium text-muted-foreground uppercase tracking-widest mb-3">Histórico de Uploads</p>
